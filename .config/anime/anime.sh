...
set_var() {
    ...
    _VIDEO_DIR_PATH="$HOME/Videos"
    _ANIME_LIST_FILE="$_VIDEO_DIR_PATH/anime.list"
...
download_file() {
    ...
    # $2: output file
    local s
    s=$("$_CURL" -sS -H "Referer: $_REFERER_URL" -H "cookie: $_COOKIE" -C - "$1" -L -g | pv -b -c -n -p -s 1000000 > "$2" \
        --connect-timeout 5 \
        --compressed \
        || echo "$?")
    if [[ "$s" -ne 0 ]]; then
...
download_episode() {
    ...
    if [[ -z ${_LIST_LINK_ONLY:-} ]]; then
        ...
        if [[ ${_PARALLEL_JOBS:-} -gt 1 ]]; then
            ...
            fname="file.list"
            cpath="$(pwd)"
            opath="$_VIDEO_DIR_PATH/$_ANIME_NAME/${num}"
